...
from ..FlowchartCtrlLibrary import FlowchartCtrlLibrary
try:
    import metaarray
    HAVE_METAARRAY = True
except:
...
def generateUi(opts):
    ...
    for opt in opts:
        ...
        if t in FlowchartCtrlLibrary.library:
            w = FlowchartCtrlLibrary.createCtrl(t, o)
        elif t == 'intSpin':
            w = QtGui.QSpinBox()
            if 'max' in o:
                w.setMaximum(o['max'])
...
class CtrlNode(Node):
    ...
    def __init__(self, name, ui=None, terminals=None, allowAddInput=False):
        if ui is None:
    ...
    def process(self, In, display=True, **kargs):
        if In is not None:
            out = self.processData(In, **kargs)
        else:
            out = None
        return {'Out': out}
